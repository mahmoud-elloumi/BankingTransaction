...
/**
 * Validates incoming transactions and routes them: - APPROVED →
 * bank.transactions.approved - REJECTED → bank.transactions.rejected
 *
 * Business rules: • Amount must be > 0 • Amount must not exceed 50,000 EUR
 * (hard limit; above goes to fraud review) • Sender must not be null for
...
public class TransactionConsumer implements Runnable {
	...
	private void process(BankTransaction tx) {
		...
		// In a real application, publish to approved/rejected topic here.
		// Example (pseudo-code):
		// approvedProducer.send(new ProducerRecord<>("bank.transactions.approved",
		// tx.getSenderId(), tx));
	}
